...
class RoIAlign3D(nn.Module):
    ...
    def __init__(self, crop_width, crop_length, crop_height, spatial_scale=1.0, extrapolation_value=0):
    ...
    def forward(self, featuremap, boxes, box_ind):
        """
        Args:
            featuremap (Tensor): Tensor of dim (N, C, W, L, H)
            boxes (Tensor): Mx6 float box with (x1, y1, z1, x2, y2, z2) **without normalization**
            box_ind (Tensor): M
        Returns: 
            Tensor of dim (M, C, oW, oL, oH)
        """

        boxes = boxes.detach().contiguous()
        box_ind = box_ind.detach()
        return CropAndResizeFunction3D.apply(featuremap, boxes, box_ind, self.crop_width, 
                                             self.crop_length, self.crop_height, self.spatial_scale,
                                             self.extrapolation_value)


def roi_align_3d(featuremap, boxes, box_ind, crop_width, crop_length, crop_height,
                 spatial_scale=1.0, extrapolation_value=0):
    boxes = boxes.detach().contiguous()
    box_ind = box_ind.detach()
    return CropAndResizeFunction3D.apply(featuremap, boxes, box_ind, crop_width, crop_length, 
                                         crop_height, spatial_scale, extrapolation_value)
